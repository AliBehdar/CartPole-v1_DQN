...
class Model_TrainTest:
    ...
    def train(self): 
        ...
        for episode in range(1, self.max_episodes+1):
            ...
            while not done and step_size<self.max_steps:
    ...
    def plot_training(self, episode):
        ...
        if self.incrising_Ep_Or_temp:
            if self.epsilon_greedy==True:
                normalized_epsilon = np.interp(self.agent.epsilon_history, (np.min(self.agent.epsilon_history), np.max(self.agent.epsilon_history)), (0, 300))
                plt.plot(normalized_epsilon, label='Normalized Epsilon', color='green', alpha=0.8)
            else:
                normalized_temperature = np.interp(self.agent.temperature_history, (np.min(self.agent.temperature_history), np.max(self.agent.temperature_history)), (0, 300))
...
if __name__ == '__main__':
    ...
    RL_hyperparams = {
        ...
        "memory_capacity"       : 5_000        if train_mode else 0,
        "max_steps"             : 300          if train_mode else 500,
        "batch_size"            : 32,
        "update_frequency"      : 40,
